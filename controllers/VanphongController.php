 ...
 */
class VanphongController extends AbstractKinhdoanhq6Controller {
    ...
    public function actionTimkiem() {
        ...
        if ($request->isGet && $request->queryParams != null) {
            $params['VanPhongSearch'] = $request->queryParams;
            $dataProvider = $model['search']->search($params);
        }
        if ($request->isPost) {
            $model['search']->load($request->post());
            if ($model['search']->ten_vanphong == NULL && $model['search']->chu_dau_tu == null && $model['search']->so_nha == NULL && $model['search']->ten_duong == NULL && $model['search']->ten_phuong == NULL) {
                return $this->redirect(Yii::$app->urlManager->createUrl('vanphong/timkiem'));
            }
            return $this->redirect(Yii::$app->urlManager->createUrl('vanphong/timkiem') .
                            '?ten_vanphong=' . $model['search']->ten_vanphong .
