...
class LeafletMap extends Component
{
    ...
    public function initListItems($url)
    {
        $js = "
        initListNocGia();
        function initListNocGia() {
            loadAjaxToDivListNocgia('$url');
            initSearchNocgia();
        };

        function loadAjaxToDivListNocgia(url) {
            var div = $('#nocgia_results');
            $.ajax({
                url: url,
                success: function (html) {
                    div.empty().append(html);
                    initPagAjaxDivListNocgia();
                    initNocgiaClickEvent();
                }
            });
        };

        function initNocgiaClickEvent() {
            $('.nocgia-item').on('click', function (e) {
                ...
                if(typeof point_x !== \"undefined\" && typeof marker_nocgia[target] !== \"undefined\"){
                    e.preventDefault();
                    $this->name.setView([point_y, point_x],18);
                    marker_nocgia[target].openPopup();
                    $.ajax({
                        url: '../ban-do/get-noc-gia?id=' + marker_nocgia[target].data.id,
                        success: function (html) {
                            $('#' + 'marker-popup-' + marker_nocgia[target].data.id).empty().append(html);
                        }
                    });
                }
            });
        ...
        function initPagAjaxDivListNocgia() {
